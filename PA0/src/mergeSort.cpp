...
void MergeSort::merge( int left, int middle1, int middle2, int right ) 
{
  ...
  while ( leftIndex <= middle1 && rightIndex <= right ){
    combined[combinedIndex++] = data[leftIndex] < data[rightIndex] ? data[leftIndex++] : data[rightIndex++];
  }
  if ( leftIndex == middle2 ){                          
    for(; rightIndex<=right; ++rightIndex, ++combinedIndex){
      combined[combinedIndex] = data[rightIndex];
    }
  } // end if                                                     
  else{                                                               
    for(; leftIndex<=middle1; ++leftIndex, ++combinedIndex){
